## functions for calculating spectral exponent over time and organizing into raster stacks 
library(tidyverse)
library(broom)
library(evobiR)
library(sp)
...
sliding_window_spec_exp <- function(file_list) {
 
  l_filenames <- file_list[[1]]
  s_filenames <- file_list[[2]]
  
  ## get path folder
  path <- str_split_fixed(l_filenames[1], pattern = "/l-detrended-", n = 2)[1,1]
  path <- str_split_fixed(path, pattern = "CMIP5-GCMs/", n = 2)[1,2]
