...
reorganize_GCM <- function(historical_filenames, rcp85_filenames, path, gcm_num) {
  ...
  while (count <= 18) {
    ...
    ## get dates from og temperature files
    if(count == 1) {
      file = 1
      dates <- c()
      while (file < length(og_names)+1) {
        nc = nc_open(paste(og_names[[file]], sep = ""))
        dates <- append(dates, ncvar_get(nc, "time"))
        nc_close(nc)
        
        file = file + 1
      }
      
      ## convert dates to commmon system
      ## note: different gcms have different Julian date systems
      if(gcm_num == 1) {
        dates <- as.Date(dates, origin = '1870-01-01')
      }
      else if(gcm_num == 2) {
        dates <- as.Date(dates, origin = '1860-01-01')
      }
      else if(gcm_num == 3) {
        dates <- as.Date(dates, origin = '1861-01-01')
      }
      else if(gcm_num == 4) {
        dates <- as.Date(dates, origin = '1859-12-01')
      }
      else if(gcm_num %in% c(5,6,7,8,9,10,11)) {
        dates <- as.Date(dates, origin = '1850-01-01')
      }
    }
      ...
      while (y < nrow(temps_df)+1) {
        ...
        if (!length(which(is.na(local_ts$temp))) == nrow(local_ts)) {
          ...
          if(length(which(is.na(local_ts$temp))) != 0 & length(which(is.na(local_ts$temp))) 
            ...
            local_ts$temp <- imputeTS::na_kalman(local_ts$temp, smooth = TRUE, model = "StructTS", type="level")
