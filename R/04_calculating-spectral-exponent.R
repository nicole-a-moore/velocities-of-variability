...
sliding_window_spec_exp <- function(path) {
  ...
  while(count < length(l_filenames)+1) {
    ...
    l_detrended_tas <- as.array(stack(l_filenames[count]))
    s_detrended_tas <- as.array(stack(s_filenames[count]))
    
    ## get lat and long bounds of chunk:
    lon_bound1 <- lon_index 
    lon_bound2 <- lon_index + 60
    lat_bound1 <- lat_index + 90
    lat_bound2 <- lat_index + 90 - 60
        ...
        if (length(which(is.na(l_local_ts))) == 83950) {
          ...
          while (n < 11) {
            ...
            while (year_start <= (2100 - n)) {
              ...
              ## store:
              spec_exp_list[[element]] <- c(l_exp, s_exp, year_start, year_stop, lat[y + lat_index],
  ...
  spec_exp_filename <- paste(path, "all-spec-exp.csv", sep = "")
