...
#################################################
###      setting paths, calling functions      ## 
#################################################
path = "range-shift-sims"
...
L = 2000 ## set length of time series 
nrow = 600
ncol = 100
...
p = c(0, 0.75, 1) ## 1 is high synchrony
p_curr = 1
...
while(beta_curr <= length(betas)) {
  ...
  all <- foreach (rep = 1:reps, .combine=rbind)  %dopar% {
    ...
    ## position optimum climatic conditions as row 50 on the lattice (Emax)
    ...
    ## replicate latitudinal gradient 500 times 
    lattice_E_it_array <- replicate(500, lattice_E_it)
    ...
    lattice_r_array = (lattice_E_it_array + lattice_ac_it[,,1:500])*r
    # plot(x = 1:nrow, y = lattice_r_array[1:nrow,1,1])
    
    ###############################################################################
    ###      run simulations at stable climate conditions for 500 time steps     ## 
    ###############################################################################
    t = 1 
    N_global <- c()
    N_ext_local <- 0
    while(t <= 500) {
      ...
      if(t == 500) {
        # ras <- raster(nrow = 150, ncols = 50, xmn = 0, xmx = 50, ymn = 0, ymx = 150)
        # values(ras) <- c(t(lattice_N_it[,,t]))
        # plot(ras)
        # values(ras) <- c(t(lattice_r_array[,,t]))
        # plot(ras)
       
        saveRDS(lattice_N_it[,,t], paste0(path, "lattice-N-it_beta_",
    ...
    ###########################################################
    ###      simulate climate shift for 1500 time steps      ## 
    ###########################################################
    ...
    lattice_E_it_shifted = lattice_E_it_array[,,1]
    ...
    while(p <= L-500) {
      ...
      ## position optimum climatic conditions as row 50 on the lattice (Emax)
      lattice_E_it_shifted[(100/2+step),] = Emax
      
      ## assume that conditions decline sigmoidally away from this optimum in both directions
      lattice_E_it_shifted[1:((100/2+step)-1),] = Emax*rev((1:((100/2+step)-1))^s/ 
                                                             ((1:((100/2+step)-1))^s + h^s))
      lattice_E_it_shifted[((100/2+step)+1):nrow,] = Emax*((1:nrow)^s / 
                                                             ((1:nrow)^s + h^s))[1:(nrow-(100/2+step))]
      # plot(x = 1:nrow, y = lattice_E_it_shifted[1:nrow,1])
        
      lattice_E_it_array[,,p] <- lattice_E_it_shifted
      
      p = p + 1
    }
    #plot(x = 1:nrow, y = lattice_E_it_array[1:nrow,1,1500])
    
    ## calculate growth rate in each cell at each time point
    lattice_r_array = (lattice_E_it_array + lattice_ac_it[,,501:L])*r
    ...
    while(t <= L) {
      ...
      ## on last step, save RDS of population sizes
      if(t == L) {
        # ras <- raster(nrow = 150, ncols = 50, xmn = 0, xmx = 50, ymn = 0, ymx = 150)
        # values(ras) <- c(t(lattice_N_it[,,t]))
        # plot(ras)
        ...
        saveRDS(lattice_N_it, paste0(path, "lattice-N-it_beta_",
                                          betas[beta_curr], "_p", p[p_curr], "_icp_", lambda[icp], 
                                          "_population_", rep, "_tALL.rds"))
