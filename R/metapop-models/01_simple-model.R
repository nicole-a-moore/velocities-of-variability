  ...
  while(beta_curr <= length(betas)) {
    ...
    all <- foreach (rep = 1:100, .combine=rbind)  %dopar% {
      ...
      while(i < L){
        ...
        curr_allpops <- sapply(N_j, function(x) x[i])
        ...
        while(y <= n_pops) {
          ...
          if(length(which(curr_allpops == 0)) == n_pops) {
            ...
            dispersers = ceiling(curr_allpops[y]*d)
            ## figure out where each one disperses 
            othercells <- 1:n_pops
            othercells <- othercells[which(othercells != y)] 
            ## subtract dispersers
            curr_allpops_new[y] <- curr_allpops_new[y] - dispersers
            
            if(length(othercells) != 0) {
              ## if some pops can still be dispersed to
              new_homes = sample(othercells, dispersers, replace = TRUE)
              ## subtract dispersers from population size, add them to population size at their new home   
              for(v in new_homes) {
                curr_allpops_new[v] = curr_allpops_new[v] + 1
              }
            }
            y = y + 1
          }
...
write.csv(all_reps_pops, "data-processed/metapop-models/simple-model_r0.8_lambda0.1_popdynam.csv",
          row.names = FALSE)
write.csv(all_reps_noise, "data-processed/metapop-models/simple-model_r0.8_lambda0.1_noise.csv",
          row.names = FALSE)

all_reps_pops <- read.csv("data-processed/metapop-models/simple-model_r0.8_lambda0.1_popdynam.csv")
...
##### GARBAGE ######
all_reps_pops$unique_id <- paste(all_reps_pops$replicate, all_reps_pops$p, all_reps_pops$beta)

split_pops <- group_split(all_reps_pops, unique_id)

beta2_p0 = filter(all_reps_pops, beta == 2, p == 0) %>%
  gather(., key = "popnum", value = "size", 
         paste0("popnum_", 1:10))

beta2_p0 %>%
  filter(replicate %in% 1:10) %>%
...
beta2_p1 = filter(all_reps_pops, beta == 2, p == 1) %>%
  gather(., key = "popnum", value = "size", 
         paste0("popnum_", 1:10))
...
beta0_p0 = filter(all_reps_pops, beta == 0, p == 0) %>%
  gather(., key = "popnum", value = "size", 
         paste0("popnum_", 1:10))
...
beta0_p1 = filter(all_reps_pops, beta == 0, p == 1) %>%
  gather(., key = "popnum", value = "size", 
         paste0("popnum_", 1:10))
