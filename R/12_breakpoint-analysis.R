...
## but this time: calculate separately for frencies >= 1 year and < 1 year
...
spectral_exponent_calculator <- function(ts_window) {
  ...
  ## 1. freq >= 1 year, freq <= 1 year
  model_output_ge1year <- spectral %>%
    filter(freq <= 1/365) %>% ## filter out frequencies less than 1 year (greater than 1/365)
    lm(., formula = log10(power) ~ log10(freq)) %>%
    tidy(.) %>%
    filter(term == "log10(freq)")
  
  model_output_le1year <- spectral %>%
    filter(freq >= 1/365) %>% ## filter out frequencies greater than 1 year
    lm(., formula = log10(power) ~ log10(freq)) %>%
    tidy(.) %>%
    filter(term == "log10(freq)")
  
  ## 2. freq > 1 year, freq < 1 year
  model_output_g1year <- spectral %>%
    filter(freq < 1/365) %>% ## filter out frequencies less than/equal to 1 year
    lm(., formula = log10(power) ~ log10(freq)) %>%
    tidy(.) %>%
    filter(term == "log10(freq)")
  
  model_output_l1year <- spectral %>%
    filter(freq > 1/365) %>% ## filter out frequencies greater than/equal to 1 year
    lm(., formula = log10(power) ~ log10(freq)) %>%
    tidy(.) %>%
    filter(term == "log10(freq)")
  
  model_estimates <- list(model_output_ge1year$estimate, model_output_le1year$estimate, 
                        model_output_g1year$estimate, model_output_l1year$estimate)
 
  return(model_estimates)
}

## function to calculate spectrogram time series window
spectrogram_calculator <- function(local_ts) {
  ...
  which(spec$f == 1/(365*2))
  plot(x = t, y = P[6,])
  
  which(spec$f == 1/(365*5))
  plot(x = t, y = P[3,])
...
sliding_window_spec_exp_breakpoint <- function(path) {
  ...
  while(count < length(l_filenames)+1) {
    ...
    while (x < ncol(l_detrended_tas)+1) {
      ...
      while (y < nrow(l_detrended_tas)+1) {
        ...
        if (length(which(is.na(l_local_ts))) == 83950) {
          ...
          #########################################
          ## calculate spectral exponent across:
          ## 1. freq >= 1 year, freq <= 1 year
          ## 2. freq > 1 year, freq < 1 year
          ## using FFT over n year windows
          ...
          while (n < 11) {
            ...
            while (year_start <= (2100 - n)) {
              ...
              ## calculate spectral exponents in window
              l_exp <- spectral_exponent_calculator(ts_chunk$l_temp)
              s_exp <- spectral_exponent_calculator(ts_chunk$s_temp)
              
              ## store: 
              spec_exp_list[[element]] <- data.frame(l_spec_exp = unlist(l_exp), 
                                         s_spec_exp = unlist(s_exp),
                                         freq_breakpoint = c(">=1y", "<=1y", ">1y", "<1y"),
                                         window_start_year = year_start,
                                         window_stop_year = year_stop,
                                         lat = lat[y + lat_index],
                                         lon = lon[x + lon_index],
    ...
    colnames(spec_exp_df) <- c("l_spec_exp", "s_spec_exp", "freq_breakpoint", "window_start_year",
                               "window_stop_year", "lat", "lon", "time_window_width")
    
    #saveRDS(spec_exp_df, "data-processed/temp_spec_exp_df.rds")

    ## convert numbers to numeric
    spec_exp_df[,c(1:2,4:7)] <- sapply(spec_exp_df[,c(1:2,4:7)], as.numeric)
    ...
    l_model_output <- spec_exp_df %>%
      group_by(lat, lon, time_window_width, freq_breakpoint) %>%
      do(tidy(lm(., formula = l_spec_exp ~ window_start_year))) %>%
      filter(term == "window_start_year")

    colnames(l_model_output)[6:9] <- paste("l", colnames(l_model_output)[6:9], sep = "_")
    ...
    colnames(s_model_output)[6:9] <- paste("s", colnames(s_model_output)[6:9], sep = "_")
    ...
    if(count == 1) {
      se_filenames <- paste(path, "spec-exp-breakpoint_long-", 
      ...
      se_filenames <- append(se_filenames,
                             paste(path, "spec-exp-breakpoint_long-", 
                                   lon_index,"-", lon_index + 60,"_lat-",
                                   90-lat_index,"-", 90-lat_index-60,  
                                   ".csv", sep = ""))
    }
...
############################################################################
###     comparing spectral exponent to breakpoint spectral exponent       ## 
############################################################################
## (for only one time chunk of air temperature data)

## read in normal spectral exponent data:
normal = read.csv("/Volumes/SundayLab/CMIP5-GCMs/01_CMCC-CESM/spec-exp_long-0-60_lat-90-30.csv")

## read in breakpoint spectral exponent data 
bp = read.csv("/Volumes/SundayLab/CMIP5-GCMs/01_CMCC-CESM/spec-exp-breakpoint_long-0-60_lat-90-30.csv")

## reorganize data a bit 
ge1y <- filter(bp, freq_breakpoint %in% c(">=1y", "<1y")) ## start with this subset 
g1y <- filter(bp, freq_breakpoint %in% c(">1y", "<=1y"))

## rename within subsets
ge1y <- ge1y %>%
  mutate(spec_exp_type = ifelse(freq_breakpoint == ">=1y", "low frequencies (>=1y)",
                                "high frequencies (<1y)"))
...
## greater than 1 y = long wavelengths, low frequency
## less than 1 y = short wavelengths, high frequency (should have negative spectral exponent)

## investigate data
## plot how low and high frequency spectral exponent differs 
ge1y %>%
  filter(time_window_width == '10 years') %>%
  ggplot(., aes(x = s_spec_exp, fill = spec_exp_type)) + geom_histogram(position = position_dodge())+
  theme_minimal()
...
ge1y %>%
  filter(time_window_width == '10 years') %>%
  ggplot(., aes(x = s_estimate, fill = spec_exp_type)) + geom_histogram(position = position_dodge()) +
  theme_minimal()
...
ge1y %>%
  filter(time_window_width == '10 years') %>%
  group_by(window_start_year, spec_exp_type) %>%
  mutate(mean_s_spec_exp = mean(s_spec_exp)) %>%
  ungroup() %>%
  select(window_start_year, mean_s_spec_exp, spec_exp_type) %>%
  unique() %>%
  ggplot(., aes(x = window_start_year, y = mean_s_spec_exp, colour = spec_exp_type)) + 
  geom_point() +
  theme_minimal() + geom_smooth(method = "lm")
## including 1 year peak makes slope positive - choose to present gy1 set 
...
## compare both to measure of overall spectral exponent 
## combine data 
normal$spec_exp_type = 'all frequencies (normal)'
normal$freq_breakpoint = 'none'

combined <- rbind(normal, g1y)
...
combined %>%
  ...
  labs(x = 'Time window start year', y = 'Mean spectral exponent',
       colour = 'Spectral exponent type:')


## magnitude of change?
combined %>%
  filter(time_window_width == '10 years') %>%
  group_by(spec_exp_type) %>%
  do(tidy(lm(data = ., s_spec_exp ~ window_start_year)))

## does direction agree locally?
agreement <- combined %>%
  filter(., time_window_width == '10 years') %>%
  select(lat, lon, s_estimate, spec_exp_type) %>%
  unique(.) %>%
  spread(key = "spec_exp_type", value = c("s_estimate")) 

colnames(agreement)[3:5] <- c("all", 'high', 'low')

agreement %>%
  mutate(in_agreement = ifelse(all >= 0 & low >=0, 'yes',
                               ifelse(all <= 0 & low <=0,
                               'yes', 'no'))) %>%
  ggplot(., aes(x = in_agreement)) + geom_bar()

agreement %>%
  mutate(diff_all_minus_low = all - high) %>%
  ggplot(., aes(x = diff_all_minus_low)) + geom_histogram()
