## visualizing the output of tos data:
library(raster)
library(tidyverse)
library(ncdf4)
...
gcm = 1
while (gcm < 10) {
  filepath = paste(folders[gcm], "sp_files_tos.rds", sep = "")
  ...
  saveRDS(s_mosaic, paste("vov-shiny/s_mosaic-", gcm_models[gcm], "_tos.rds", sep = ""))
  saveRDS(l_mosaic, paste("vov-shiny/l_mosaic-", gcm_models[gcm], "_tos.rds", sep = ""))
