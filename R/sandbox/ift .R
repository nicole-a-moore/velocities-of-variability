  ...
  while (x < ncol(detrended_tas)+1) {
    ...
    while(y < nrow(detrended_tas)+1) {
      ...
      if (length(which(is.na(local_ts))) == 51830) {
        ...
        ## inverse Fourier transform the temperatures 
        idft <- fft(dft, inverse = T) ## inverse fast fourier transform the coefficients to get the temporal noise series
        noise = rev(as.numeric(idft[1:51830]))
...
data.frame(x = 1:length(noise), noise = noise, local_ts = local_ts, dif = abs(local_ts - noise)) %>%
  ggplot(aes(x=x, y  = dif)) + geom_line() 
...
while (n < 11) {
  ...
  while (year_start <= (2020 - n)) {
    ...
    if(!any(is.na(ts_chunk$true_ts))) {
      ...
      ts_noise <- ts_chunk$noise - mean(ts_chunk$noise)
