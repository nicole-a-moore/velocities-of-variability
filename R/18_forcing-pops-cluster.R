  ...
  while(col <= length(start_cols)) {
    ...
    while (icp <= length(lambda)) {
      ...
      if(i <= length(incs)) {
        ...
        inc_sims = run_100_sims(inc = TRUE, 
                                   start_colour = start_colour,
                                   end_colour = end_colour, 
                                   Tmax = Tmax,
                                   ICP = lambda[icp])
        ...
        write.csv(inc_sims, paste("pop-sims/tvfdpopdynam_icp-", lambda[icp], "_change-", 
                                  incs[i], "_start-col-", start_colour, "_20-steps.csv", sep = ""), row.names = F)
      }
      
      if(i <= length(decs)) {
        end_colour = start_colour + decs[i]
        
        dec_sims = run_100_sims(inc = FALSE, 
                                   start_colour = start_colour,
                                   end_colour = end_colour, 
                                   Tmax = Tmax,
                                   ICP = lambda[icp])
        ...
        write.csv(dec_sims, paste("pop-sims/tvfdpopdynam_icp-", lambda[icp], "_change-", 
                                  decs[i], "_start-col-", start_colour, "_20-steps.csv", sep = ""), row.names = F)
      }
